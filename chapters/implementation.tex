...
used to get a list of all the item names. Unfortunately, the same does not exist for trinkets and characters as the pages that would 
contain that list instead use a template which autofills the data. As a temporary fix, the list of trinkets is fetched from a hardcoded file 
and the character names are fetched from the \verb|Custom Challenges| page. In the future this could be updated to scrape that information from the website.
\begin{figure}[H]
